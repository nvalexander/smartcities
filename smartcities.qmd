...
Manually-loaded data was pasted in LibreOffice Calc 25.2.2, and the resulting raw spreadsheet has been shared at TODO. Subsequently, data was imported in VSCode 1.99.3, processed and analyzed with R `r getRversion()` (@RCoreTeamlanguageenvironmentstatistical2021), and typeset with Quarto `r quarto::quarto_version()`. Data wrangling and visualization relied on the tidyverse R library, version `r as.character(packageVersion("tidyverse"))` (@tidyverse2019).
...
Although USNW and Numbeo.com provide QoL ratings for hundreds and, respectively thousands of US cities, the number of cities ranked by IESE and IMD is typically around 100 across the world. For the most part we analyzed data for all the 19 US cities featured the IESE 2024 rankings, but only 4 US cities (New York, Los Angeles, Chivago and Philadelphia) were assessed in all three reports used for this study. A set of 76 variables were obtained, including 59 metrics of SDC from IMD, 4 SCD metrics from IESE, 9 QoL metrics from Numbeo.com, 2 QoL metrics from USNW, and a control variable (overall USNW score).
```{r qc,echo=FALSE}

data_qc <- alldata %>% select(-City, -ScoreUSNewsoverall)

n_vars <- ncol(data_qc)
n_obs <- nrow(data_qc)
missing_summary <- data_qc %>% summarise(across(everything(), ~ mean(is.na(.)))) %>% pivot_longer(everything())
...
ieserank2024 <- data_qc[["RankIESE2024"]]
imdrank2023  <- data_qc[["RankIMD2023"]]
iesescore2024 <- data_qc[["CIMI2024"]]
...
cor_imdrank_2019_2020 <- cor.test(data_qc$RankIMD2019, data_qc$RankIMD2020, method = "pearson", use = "pairwise.complete.obs")
cor_imdrank_2020_2021 <- cor.test(data_qc$RankIMD2020, data_qc$RankIMD2021, method = "pearson", use = "pairwise.complete.obs")
cor_imdrank_2021_2023 <- cor.test(data_qc$RankIMD2021, data_qc$RankIMD2023, method = "pearson", use = "pairwise.complete.obs")

icc_imdrank_result <- icc(
  data_qc %>% select(RankIMD2019, RankIMD2020, RankIMD2021, RankIMD2023),
  model = "oneway",
  type = "consistency"
)
```
...
cor_hdi_tech <- cor.test(data_qc$HDI, data_qc$IMD_Technologies_Sum, method = "pearson", use = "pairwise.complete.obs")
cor_hdi_struct <- cor.test(data_qc$HDI, data_qc$IMD_Structures_Sum, method = "pearson", use = "pairwise.complete.obs")
cor_tech_struct <- cor.test(data_qc$IMD_Technologies_Sum, data_qc$IMD_Structures_Sum, method = "pearson", use = "pairwise.complete.obs")

cor_hdi_concerns <- cor.test(data_qc$HDI, data_qc$IMD_Concerns_Sum, method = "spearman", use = "pairwise.complete.obs")
cor_tech_concerns <- cor.test(data_qc$IMD_Technologies_Sum, data_qc$IMD_Concerns_Sum, method = "spearman", use = "pairwise.complete.obs")
cor_struct_concerns <- cor.test(data_qc$IMD_Structures_Sum, data_qc$IMD_Concerns_Sum, method = "spearman", use = "pairwise.complete.obs")
...
Of note, the IMD Concerns sum significantly from normality (p = `r round(shapiro.test(data_qc$IMD_Concerns_Sum)$p.value, 3)`), unlike the other three IMD domains. Therefore, for associations involving the non-normally distributed IMD Concerns sum, Spearman correlations were used instead. Evidence of a correlation between HDI and IMD Concerns was found using Spearman's method (ρ = `r round(cor_hdi_concerns$estimate, 2)`, p = `r round(cor_hdi_concerns$p.value, 3)`). On the other hand, there was no evidence for an association between Technologies and Concerns (ρ = `r round(cor_tech_concerns$estimate, 2)`, p = `r round(cor_tech_concerns$p.value, 3)`), nor for the association between Structures and Concerns (ρ = `r round(cor_struct_concerns$estimate, 2)`, p = `r round(cor_struct_concerns$p.value, 3)`).
...
p1imdcomponents <- ggplot(
  ...
  scale_y_continuous(limits = c(445, 470), breaks = seq(450, 470, 10)) +
  geom_smooth(method = "lm", se = TRUE, color = "grey", fill = "lightgrey", alpha = 0.2) +
  labs(x = "HDI", y = "IMD Concerns") +
  geom_text_repel(size = 1) +
  theme_minimal(base_size = 8)

p2imdcomponents <- ggplot(data_qc, aes(x = IMD_Technologies_Sum, y = IMD_Concerns_Sum)) +
  geom_point(size = 1.5) +
...
p3imdcomponents <- ggplot(data_qc, aes(x = IMD_Structures_Sum, y = IMD_Concerns_Sum)) +
  geom_point(size = 1.5) +
...
p4imdcomponents <- ggplot(data_qc, aes(x = HDI, y = IMD_Structures_Sum)) +
  geom_point(size = 1.5) +
  scale_x_continuous(limits = c(0.907, 0.95), breaks = seq(0.91, 0.95, 0.01)) +
...
p5imdcomponents <- ggplot(data_qc, aes(x = IMD_Technologies_Sum, y = IMD_Structures_Sum)) +
  geom_point(size = 1.5) +
  scale_x_continuous(limits = c(960, 1280), breaks = seq(960, 1280, 80)) +
  scale_y_continuous(limits = c(880, 1120), breaks = seq(880, 1120, 40)) +
  geom_smooth(method = "lm", se = TRUE, color = "grey", fill = "lightgrey", alpha = 0.2) +
...
p6imdcomponents <- ggplot(data_qc, aes(x = HDI, y = IMD_Technologies_Sum)) +
  geom_point(size = 1.5) +
  scale_x_continuous(limits = c(0.907, 0.95), breaks = seq(0.91, 0.95, 0.01)) +
