...
Berrone et al. (\citeproc{ref-BerroneIESECitiesMotion2024}{2024}) in
...
\CommentTok{\# List only sheets ending in IMD}
\NormalTok{sheets }\OtherTok{\textless{}{-}} \FunctionTok{excel\_sheets}\NormalTok{(}\StringTok{"data/rawdata.xlsx"}\NormalTok{)}
\NormalTok{imd\_sheets }\OtherTok{\textless{}{-}}\NormalTok{ sheets[}\FunctionTok{str\_ends}\NormalTok{(sheets, }\StringTok{"IMD"}\NormalTok{)]}

\CommentTok{\# Function to extract last positive number from first column}
\NormalTok{extract\_percents }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(sheet) \{}
\NormalTok{  df }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}\StringTok{"data/rawdata.xlsx"}\NormalTok{, }\AttributeTok{sheet =}\NormalTok{ sheet)}
\NormalTok{  first\_col }\OtherTok{\textless{}{-}}\NormalTok{ df[[}\DecValTok{1}\NormalTok{]]  }\CommentTok{\# Take the actual first column, whatever its name}
  \FunctionTok{tibble}\NormalTok{(}\AttributeTok{percent =} \FunctionTok{str\_extract}\NormalTok{(first\_col, }\StringTok{"}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{d+}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{.?}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{d*$"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{percent =} \FunctionTok{as.numeric}\NormalTok{(percent)) }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{pull}\NormalTok{(percent)}
\NormalTok{\}}

\CommentTok{\# Apply to each sheet}
\NormalTok{results }\OtherTok{\textless{}{-}} \FunctionTok{map}\NormalTok{(imd\_sheets, extract\_percents)}
\end{Highlighting}
\end{Shaded}
